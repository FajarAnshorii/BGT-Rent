import { SERVICES, WHATSAPP_LINK, WHATSAPP_MESSAGES } from '@/lib/constants'
import { Calendar, CalendarRange, Factory, Building2, Car, MessageCircle, LucideIcon } from 'lucide-react'

const SERVICE_ICONS: Record<string, LucideIcon> = {
  Calendar,
  CalendarRange,
  Factory,
  Building2,
  Car,
  MessageCircle,
}

export default function Layanan() {
  return (
    <section id="layanan" className="section-padding bg-white">
      <div className="container-custom">
        {/* Section Header */}
        <div className="text-center mb-12">
          <span className="inline-block px-4 py-1.5 bg-primary-100 text-primary-700 rounded-full text-sm font-medium mb-4">
            Layanan Kami
          </span>
          <h2 className="section-title">Layanan Rental Kendaraan</h2>
          <p className="section-subtitle mx-auto">
            Berbagai pilihan layanan rental kendaraan untuk memenuhi kebutuhan operasional Anda
          </p>
        </div>

        {/* Services Grid */}
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {SERVICES.map((service) => {
            const IconComponent = SERVICE_ICONS[service.icon] || Car
            return (
              <div
                key={service.title}
                className="card group hover:shadow-lg hover:border-primary-200 border border-gray-100"
              >
                <div className="w-14 h-14 bg-gradient-to-br from-primary-400 to-primary-600 rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                  <IconComponent className="w-7 h-7 text-white" />
                </div>
                <h3 className="text-xl font-bold text-gray-900 mb-2">{service.title}</h3>
                <p className="text-gray-600">{service.description}</p>
              </div>
            )
          })}
        </div>

        {/* Note */}
        <div className="mt-12 max-w-3xl mx-auto">
          <div className="bg-gradient-to-r from-primary-50 to-primary-100 rounded-xl p-6 text-center">
            <p className="text-gray-700">
              <strong className="text-primary-700">Catatan:</strong> Harga sewa dapat dinegosiasikan berdasarkan jenis kendaraan, durasi sewa, rute, kebutuhan driver, dan cakupan operasional.
            </p>
          </div>
        </div>

        {/* CTA */}
        <div className="mt-12 text-center">
          <a
            href={`${WHATSAPP_LINK}?text=${encodeURIComponent(WHATSAPP_MESSAGES.armada)}`}
            target="_blank"
            rel="noopener noreferrer"
            className="btn-primary"
          >
            <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
            </svg>
            Konsultasi Kebutuhan Armada
          </a>
        </div>
      </div>
    </section>
  )
}
